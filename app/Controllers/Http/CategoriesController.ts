...
export default class CategoriesController {
    ...
    try {
      ...
      if (params.level && !params.id) {
        ...
        const query = await db.from('categorie_ones')
          ...
          .select(
            'categorie_twos.libelle AS level2',
            'categorie_twos.color AS level2_color',
            'categorie_twos.id AS level2_id'
          )
          .select(
            'categorie_threes.libelle AS level3',
            'categorie_threes.color AS level3_color',
            'categorie_threes.id AS level3_id'
          )
  ...
  public async getByLibelle({ params, response }) {
    ...
    categorie = await CategorieOne.query()
      .whereRaw('LOWER(libelle) = LOWER(?)', [params.libelle])
      .first()
    if (!categorie) {
      categorie = await CategorieTwo.query()
        .whereRaw('LOWER(libelle) = LOWER(?)', [params.libelle])
        .first()
    }
    if (!categorie) {
      categorie = await CategorieThree.query()
        .whereRaw('LOWER(libelle) = LOWER(?)', [params.libelle])
        .first()
    }
    ...
    if (!categorie) {
      return response
        .status(422)
        .send({ message: `Aucune catégorie ne correspond au libelle : ${params.libelle}` })
    }
    return categorie
  }
    ...
    try {
      ...
      if ((data.level && data.level == 1) || (data.id && !data.level)) {
        ...
        if (data.id) {
          ...
          categorie = await CategorieOne.query()
            .whereRaw('LOWER(libelle) = LOWER(?)', [data.libelle])
            .first()
        }
      ...
      if (data.level == 2) {
        ...
        if (!categorieParent && data.libelle_parent) {
          categorieParent = await CategorieOne.query()
            .whereRaw('LOWER(libelle) = LOWER(?)', [data.libelle_parent])
            .first()
        }
        ...
        if (!categorieParent) {
          return response
            .status(422)
            .send({ message: 'Aucune catégorie parente trouvé avec cet identifiant et/ou libellé' })
        }
      ...
      if (data.level == 3) {
        ...
        // Ou a défaut si le libelle existe
        if (!categorieParent && data.libelle_parent) {
          categorieParent = await CategorieTwo.query()
            .whereRaw('LOWER(libelle) = LOWER(?)', [data.libelle_parent])
            .first()
        }
